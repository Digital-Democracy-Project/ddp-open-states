...
- **This entire STOP section assumes you're a human-driven, persistent session on this specific
  Mac Studio, using the fixed local paths named above.** If you're an automated pipeline (e.g.
  CodeBot) running inside your own disposable workspace clone of this repo — a fresh checkout
  under a path like `~/codebot/workspace/{task_id}/...` that gets thrown away after the task —
  that clone is already your isolated sandbox. Make your changes directly in it, branch and open
  your PR from there as normal, and do not look for or reference
  `~/Developer/repos/ddp-open-states-dev` — that path is a separate, human-owned local checkout on
  this one machine that your pipeline has no knowledge of and no business touching. (Found
  2026-08-01, OPEN-17: an automated CodeBot run took this section literally and pushed a real,
  correct fix into the human's own dev checkout under the human's own git identity instead of its
  own workspace, orphaning the fix from CodeBot's PR/reporting pipeline entirely.)
...
**This dev/prod split is local-checkout-only — it has no meaning for an automated pipeline's own
...
is a third thing entirely — already isolated by construction, safe to edit directly, and entirely
unrelated to either fixed local path above. See the STOP section's note on this, above.
